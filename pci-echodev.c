/* ***************************************************
* QEMU Source Code in /qemu/hw/misc
* File        : pci-echodev.c 
* Description : A character device driver for QEMU PCI DEVICE.
*               youtube.com/watch?v=MTUuymrutNw
* Author      : Steve Scanlon
* Date        : 08 May 2024
* Credits     : Based on a series of tutorials by 
*               Johannes 4GNU_Linux on YouTube.
*
* github = https://github.com/Johannes4Linux/pci-echodev/blob/main/pci-echodev.c
* 
* *****************************************************/
...
#define INV_REGISTER     0x04
#define IRQ_REGISTER     0x08
#define RANDVAL_REGISTER 0x0C
#define DMA_SRC          0x10
#define DMA_DST          0x18
#define DMA_CNT          0x20 
#define DMA_CMD          0x28
...
static void pciechodev_bar0_mmio_write(void *opaque, hwaddr addr, uint64_t val, unsigned size)
{
    printf("PCIECHODEV : BAR0 pciechodev_bar0_mmio_write() addr %lx size %x val %lx \n", addr, size, val);
    PciechodevState *pciechodev = opaque;
...
{
    PciechodevState *pciechodev = opaque;
    printf("PCIECHODEV: BAR 1 pciechodev_mmio_bar1_read() addr %lx size %x \n", addr, size);
...
{
    printf("PCIECHODEV: BAR1 pciechodev_bar1_mmio_write() addr %lx size %x val %lx \n", addr, size, val);
    PciechodevState *pciechodev = opaque;
...
static void pci_pciechodev_realize(PCIDevice *pdev, Error **errp)
{
    ...
    // Access to this region will cause the callbacks
    // of the pciechodev_mmio_ops to be called.

    memory_region_init_io(&pciechodev->mmio_bar0, OBJECT(pciechodev), &pciechodev_bar0_mmio_ops, pciechodev, "pciechodev-mmio", 64); //note was 16

    // registering the pdev and all of teh above configuration
